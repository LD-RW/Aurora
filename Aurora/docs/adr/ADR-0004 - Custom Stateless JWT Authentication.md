...
- **Library-backed cryptography:** signing, parsing, and signature verification are delegated to `jjwt` rather than hand-rolled, reducing the chance of introducing a cryptographic or encoding bug.
...
- **No refresh-token flow:** with only a 5-minute expiration and no refresh mechanism, clients must re-authenticate with credentials every 5 minutes. This is acceptable for early development/testing but isn't viable as-is for a real client experience and will need a refresh-token (or sliding-session) design before this ships to real users.
...
- **Header-based delivery is an interim state:** delivering the token via `Authorization: Bearer` means whatever the client does with it (for example, storing it in `localStorage`) determines its exposure to XSS-based token theft. Migrating delivery to a `Secure`/`HttpOnly` cookie is already planned as follow-up work; until then, client-side token storage choices are a live risk this ADR doesn't resolve.
...
Wiring the security filter chain surfaced a defect in the signing secret. The "Secret is a weak, committed plaintext value" trade-off in the Negative consequences no longer applies as first written. The secret is now hardened in two ways.

- **Key strength.** `spring.app.jwtSecret` now holds a Base64-encoded 512-bit random key. This satisfies the 256-bit minimum that RFC 7518 (Section 3.2) sets for HMAC-SHA. The earlier value decoded to 112 bits, which made `jjwt` raise a `WeakKeyException` on every token operation. Token issuance and validation couldn't have worked with that value.
- **Externalized configuration.** The secret and the expiration read from the `JWT_SECRET` and `JWT_EXPIRATION_MS` environment variables, with the committed values kept only as fallbacks. A deployment supplies its own key without editing source.

Part of the original trade-off still stands. The committed default value remains in `application.properties` for local development only. A production deployment must set `JWT_SECRET` from the environment or a secrets manager, and must not use the committed default. Asymmetric signing and the missing revocation and refresh-token flows remain open, as described above.

## Update (2026-08-02)

The planned follow-up named in "Delivery mechanism" above has landed. Aurora now delivers the JWT through a `Secure`, `HttpOnly`, `SameSite=Strict` cookie instead of the `Authorization: Bearer` header, closing the "Header-based delivery is an interim state" item from the original Negative consequences.

- **Mechanism.** `JwtUtils.generateJwtCookie` builds the cookie on `/api/auth/signin`; `AuthTokenFilter` reads it back through `JwtUtils.getJwtFromCookie` (`WebUtils.getCookie`) instead of parsing an `Authorization` header. `getJwtFromHeader` is gone, and there's no remaining header-based path.
- **Cookie attributes.** `HttpOnly` keeps the token out of reach of `document.cookie`, closing the XSS-exposure gap named in the original trade-off. `Secure` restricts the cookie to HTTPS requests (browsers treat `localhost` as a secure context, so local HTTP development still works). `SameSite=Strict` stops the browser from attaching the cookie to cross-site requests, the primary mitigation for the CSRF risk this change introduces (see below).
- **Lifetime stays in sync.** The cookie's `maxAge` is derived from `spring.app.jwtExpirationMS` (`jwtExpirationMs / 1000`) instead of a separate, hard-coded value, so the cookie and the token it carries always expire together.
- **New trade-off: CSRF.** Header-based delivery was immune to CSRF, since browsers don't attach custom headers to cross-site requests automatically; a cookie is attached automatically. `SameSite=Strict` mitigates this for modern browsers, but Spring Security's CSRF protection stays disabled (`WebSecurityConfig`'s `csrf.disable()`), so there's no CSRF-token double-submit as a second layer. This is a narrower, accepted risk compared to before the change (there was no cookie to protect previously), not a fully closed one—revisit if a future cross-origin frontend needs `SameSite` relaxed from `Strict` to `Lax` or `None`.
- **The response body no longer carries the token.** `UserInfoResponse.jwtToken` is removed; signin responses now return only `id`, `username`, and `roles`. A client can no longer read the raw token from the response body—retrieving it needs cooperation from the browser's cookie jar, which `HttpOnly` blocks from script access entirely.

This satisfies the "already-planned follow-up" note from the original decision. The remaining open items from the original Negative consequences (no revocation, no refresh-token flow, symmetric signing) are unchanged by this update.
